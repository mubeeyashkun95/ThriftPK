...
const menuItems = [
  ...
  { name: 'Shop', path: '/shop', icon: 'shopping-bag' as const },
  { name: 'About', path: '/about', icon: 'information-circle' as const },
  { name: 'Contact', path: '/contact', icon: 'chat-bubble' as const },
]
      ...
      <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
          ...
          <router-link to="/" class="flex items-center gap-2 text-2xl font-bold transition-all hover:scale-105">
            <img src="/src/assets/logo2.png" class="w-20 drop-shadow-2xl" alt="StepStyle Logo">
          </router-link>
        ...
        >
          <div class="absolute top-0 right-0 h-full border-l shadow-2xl w-80 bg-white/95 backdrop-blur-2xl border-white/30">
            <div class="flex flex-col items-center gap-8 pt-24 pb-12">
              ...
              >
                <component :is="icons[item.icon as IconKey]" class="w-8 h-8 text-blue-600" />
                {{ item.name }}
              </router-link>

              <button
                @click="openWhatsApp(); mobileMenuOpen = false"
                class="w-3/4 py-4 mt-8 text-lg font-bold text-white shadow-xl rounded-2xl bg-gradient-to-r from-green-500 to-green-600 hover:scale-105"
              >
                Chat on WhatsApp
              </button>
            </div>
